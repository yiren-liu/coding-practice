...
def sliding_window_attention(
    ...
    Returns:
        ...
        attention_weights: Attention weights of shape [batch_size, num_heads, seq_len, 2*window_size+1]
                          Note: This is smaller than full attention matrix!
    """
    # TODO: Implement sliding window attention
    # 
    # Steps:
    # 1. Create a sliding window mask (band diagonal matrix)
    #    - Each position i can attend to positions [i-window_size, i+window_size]
    #    - Use torch.triu and torch.tril to create a band matrix
    #
    # 2. Compute attention scores: QK^T / sqrt(d_k)
    #
    # 3. Apply the sliding window mask
    #    - Set positions outside the window to -inf before softmax
    #
    ...
    #
    # Hint: For efficiency, you can compute full attention and mask it,
    #       or extract only the relevant window positions (more complex but faster)
