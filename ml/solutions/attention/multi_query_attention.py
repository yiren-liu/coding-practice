...
class MultiQueryAttention(nn.Module):
    ...
    def __init__(
        ...
        # Key and Value projections: SINGLE shared projections
    ...
    def forward(
        ...
        K = self.W_k(x)  # [batch_size, seq_len, d_k] - SHARED across all heads
        V = self.W_v(x)  # [batch_size, seq_len, d_k] - SHARED across all heads
        
        # 2. Reshape Q to have multiple heads
        # Q: [batch_size, seq_len, d_model] -> [batch_size, num_heads, seq_len, d_k]
        Q = Q.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
        
        # K and V: Add head dimension for broadcasting
        # [batch_size, seq_len, d_k] -> [batch_size, 1, seq_len, d_k]
        # The dimension of size 1 will broadcast across all query heads
        K = K.unsqueeze(1)  # [batch_size, 1, seq_len, d_k]
        V = V.unsqueeze(1)  # [batch_size, 1, seq_len, d_k]
        
        # 3. Compute scaled dot-product attention
        # Q: [batch_size, num_heads, seq_len, d_k]
        # K: [batch_size, 1, seq_len, d_k] - broadcasts to [batch_size, num_heads, seq_len, d_k]
        ...
        # V: [batch_size, 1, seq_len, d_k] - broadcasts to [batch_size, num_heads, seq_len, d_k]
